...
void child_process(int process_number, int* columns_number, int* first_columns, int max_time, int mode, int pairs, char* list){
...
int main(int argc, char* argv[]){
    if(argc < 5){
        ...
        printf("-----------------------------------------\n");
        printf("Usage: macierz <list> <num_of_processes> <max_time> <type>\n");
    ...
    for(int p=0; p< num_of_processes; p++){
        ...
        if(child_pid!=0){
            ...
            child_process(p, column_count[p], first_column[p], max_time, type, pairs, list);
        }
