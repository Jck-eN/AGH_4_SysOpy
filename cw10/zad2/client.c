...
void board_check() {
    ...
    if (is_drawn && !game_is_finished) {
        printf("Gra skończona. Remis!\n");
    }

    if (game_is_finished || is_drawn) {
        sleep(1);
        state = QUIT;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(connection_mode, "local") == 0) {
        ...
        struct sockaddr_un binded_socket_address;
        memset(&binded_socket_address, 0, sizeof(struct sockaddr_un));
        binded_socket_address.sun_family = AF_UNIX;
        sprintf(binded_socket_address.sun_path, "/tmp/%d", getpid());

        bind(binded_socket, (struct sockaddr*)&binded_socket_address,
             sizeof(struct sockaddr_un));
    }
    else {
    ...
    if (islocal) {
        sendto(binded_socket, msg_buffer, MESSAGE_LENGTH_MAX, 0,
               (struct sockaddr*)local_socket_addr, sizeof(struct sockaddr_un));
    } else {
    ...
    while (1) {
        if (islocal) {
            recv(binded_socket, msg_buffer, MESSAGE_LENGTH_MAX, 0);
        } else {
